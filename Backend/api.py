...
def courseSignup(course_id):
    ...
    if request.method == 'POST':
        ...
        with db.engine.connect() as conn:
            conn.execute(insertToCourseGrade, {'course_id': course_id, 'grade': 0, 'user_id': user_id})
            conn.execute(insertToStudentCourseGrade, {'course_id': course_id, 'grade': 0, 'user_id': user_id})
            conn.commit()
